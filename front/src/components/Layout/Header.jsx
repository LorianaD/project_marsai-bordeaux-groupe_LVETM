      ...
      >
        ...
        <nav className="hidden md:block">
          <ul className="flex items-center justify-center gap-[41px] text-[16px] font-bold leading-[15px] tracking-[3px] uppercase">
            <li>
              <Link to={content?.[section]?.home_link || "/"}>
                {content?.[section]?.home || t("home")}
              </Link>
            </li>

            {isVisible(content, section, "first") &&
              firstLabel &&
              firstLink && (
                <li>
                  <Link to={firstLink}>{firstLabel}</Link>
                </li>
              )}

            {isVisible(content, section, "seconde") &&
              secondeLabel &&
              secondeLink && (
                <li>
                  <Link to={secondeLink}>{secondeLabel}</Link>
                </li>
              )}

            {isVisible(content, section, "third") &&
              thirdLabel &&
              thirdLink && (
                <li>
                  <Link to={thirdLink}>{thirdLabel}</Link>
                </li>
              )}
          </ul>
        </nav>
        ...
        <div className="hidden md:flex items-center justify-center gap-[34px]">
          {isVisible(content, section, "btn") && btnLabel && btnLink && (
            <Link
              to={btnLink}
              className="flex items-center justify-center gap-[10px] px-[40px] py-[10px] rounded-[20px] bg-[linear-gradient(90deg,#2B7FFF_0%,#9810FA_100%)] text-white text-center text-[16px] font-bold leading-[16px] uppercase"
            >
              {btnLabel}
            </Link>
          )}
